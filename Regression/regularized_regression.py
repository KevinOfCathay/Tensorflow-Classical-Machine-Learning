...
penalty = tensorflow.constant(0.5)
regularization = tensorflow.multiply(
    penalty, tensorflow.reduce_mean(tensorflow.abs(W)))
y_predict = tensorflow.add(tensorflow.add(
    tensorflow.matmul(x_in, W), b), regularization)

loss_func = tensorflow.reduce_mean(tensorflow.square(y_in - y_predict))
